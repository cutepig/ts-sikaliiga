...
export function simulateMHM2K(game: IGame, match: IMatchSimulation): IMatchSimulation {
  ...
  function getRandomFieldPlayer(players: IMatchSimulationPlayer[]) {
    let player: IMatchSimulationPlayer;
    do {
  ...
  function getAssists(shooter: IMatchSimulationPlayer, players: IMatchSimulationPlayer[]) {
    const assists: IMatchSimulationPlayer[] = [];
  ...
  function calculateTeamStats(
    players: IMatchSimulationPlayer[],
    goalie: IMatchSimulationPlayer | undefined,
    ...
    let events: IMatchSimulationEvent[] = [];
  ...
  function calculateFieldValue(field: Field, mapFn: (player: IPlayer) => number) {
    return field.reduce((acc, playerId) => {
      ...
      return acc + (player ? mapFn(player) : 0);
    }, 0);
  }
  ...
  function calculateFieldValues(teamId: string, fields: IFields): ITeamValues {
    ...
    const d = fields.defense
      ...
      .reduce(
        (acc, field, index) =>
          acc +
          (calculateFieldValue(field, player => player.defense) * defenseFieldWeights[index]) / 2,
        0,
      );
    const a = fields.forwards
      ...
      .reduce(
        (acc, field, index) =>
          acc +
          (calculateFieldValue(field, player => player.attack) * forwardFieldWeights[index]) / 3,
        0,
      );
